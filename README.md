# comfy-groqchat
目前节点的对话功能基于Groq API（ https://console.groq.com/keys ）和  kimi API ( https://platform.moonshot.cn/console/api-keys )， 请在相应的网站获取，  填入到将新的api放到这个文件中 api_key.ini.example  后缀改成.ini文件中。通过API的调用，可以实现单轮或者多轮chat，以实现正反提示词生成和报错指南等，此外节点还整合了基于moondream2和PaliGemma大模型的的图片反推节点。
<details>
